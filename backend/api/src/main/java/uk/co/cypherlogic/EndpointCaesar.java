...
/**
 * REST API Endpoint for Caesar cipher
 *
 * @author ESRS Group 2
 * @version 2022-02-16
 */
@Path("/caesar")
public class EndpointCaesar {
    ...
    public Response encrypt(@PathParam("pt") String plaintext, @PathParam("key") String key) {
        ...
        if (key.length() != 1) {
            return Response.ok(badParams.toString()).header("Access-Control-Allow-Origin", "*").build();
        }
